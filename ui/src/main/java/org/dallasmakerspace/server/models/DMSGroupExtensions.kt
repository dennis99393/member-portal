...
/** Extension functions and utilities for DMSGroup */

/** URL slug (URL Fragment) for the group name. */
val DMSGroup.slug: String
  get() = getSlugFromName(this.name)

/**
 * Generate a slug (URL Fragment) from a group name. The group name can have spaces that get
 * translated to %20 which results in an ugly URL. So we replace space with "+" to use in the URL.
 * [getNameFromSlug] performs the reverse process.
 */
fun getSlugFromName(groupName: String) = groupName.replace(" ", "+").encodeURLPath()

/**
 * Generate a group name from a slug (URL Fragment). The slug can have "+" that get translated to
 * spaces. [getSlugFromName] performs the reverse process.
 */

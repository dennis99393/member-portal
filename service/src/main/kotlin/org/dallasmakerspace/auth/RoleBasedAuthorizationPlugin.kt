...
/**
 * Plugin that performs role-based authorization by checking if the authenticated principal has at
...
class RoleBasedAuthorizationPlugin(private val requiredRoles: Set<String>) {
  ...
  class Configuration {
    ...
    /**
     * Add a required role. The authenticated principal must have at least ONE of the specified
...
fun Route.requireRoles(vararg roles: String, build: Route.() -> Unit): Route {
  ...
  authorizedRoute.install(RoleBasedAuthorizationPlugin) { roles(*roles) }

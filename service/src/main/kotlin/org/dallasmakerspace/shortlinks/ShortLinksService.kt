...
/**
 * Business logic for short links feature.
 * Handles validation, slug generation, and redirect resolution.
 */
class ShortLinksService @Inject constructor(
    loggerFactory: LoggerFactory,
    private val repository: ShortLinksRepository
) {
    ...
    suspend fun createNamespace(
        ...
        validateAlias(primaryAlias)?.let { return Result.failure(it) }

        // Validate additional aliases
        additionalAliases.forEach { alias ->
            validateAlias(alias)?.let { return Result.failure(it) }
        }

        // Create namespace
        val namespace = repository.createNamespace(
            Namespace(
                id = -1,
                name = name,
                ownerType = ownerType,
        ...
        log.info("Created namespace '${namespace.name}' (ID: ${namespace.id}) with primary alias '$primaryAlias'")
    ...
    suspend fun updateNamespace(id: Int, updates: Namespace): Result<Namespace> {
        val updated = repository.updateNamespace(id, updates)
    ...
    suspend fun addAlias(namespaceId: Int, alias: String, isPrimary: Boolean): Result<NamespaceAlias> {
        validateAlias(alias)?.let { return Result.failure(it) }
    ...
    suspend fun createShortLink(
        ...
        validateUrl(destinationUrl)?.let { return Result.failure(it) }

        // Generate slug if not provided (root-level only)
        val finalSlug = if (slug != null) {
            // Custom slug - validate it
            validateSlug(slug)?.let { return Result.failure(it) }
            slug
        } else {
        ...
        val link = repository.createShortLink(
            ShortLink(
                id = -1,
        ...
        log.info("Created short link: ${formatShortLinkPath(namespaceId, finalSlug)} -> $destinationUrl")
    ...
    suspend fun getAllShortLinks(namespaceId: Int? = null, activeOnly: Boolean = true): List<ShortLink> {
    ...
    suspend fun updateShortLink(id: Int, updates: ShortLink): Result<ShortLink> {
        ...
        validateSlug(updates.slug)?.let { return Result.failure(it) }

        // Validate URL if changed
        validateUrl(updates.destinationUrl)?.let { return Result.failure(it) }

        val updated = repository.updateShortLink(id, updates)
    ...
    suspend fun resolveRedirect(path: String, memberId: Int? = null): RedirectResult {
        ...
        val (namespaceId, slug) = if (parts.size == 2) {
            // Namespace path: "ws/safety-video"
            val alias = parts[0]
            val resolvedId = repository.resolveAlias(alias)
        ...
        // Look up short link
        val link = repository.getShortLinkBySlug(namespaceId, slug)
